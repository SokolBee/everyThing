    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        for (int i = 0; i < humanRelationships.length; i++) {
            for (int j = 0; j < humanRelationships[index].length; j++) {
                if (humanRelationships[index][j] && j != index) {
                    result.add(j);
                    if (deep != 0) {
                        result.addAll(getAllFriendsAndPotentialFriends(j, deep));
                    }
                }
            }
            if (index < humanRelationships[i].length && humanRelationships[i][index] && i != index) {
                result.add(i);
                if (deep != 0) {
                    result.addAll(getAllFriendsAndPotentialFriends(i, deep));
                }
            }
        }
        result.remove(index);
